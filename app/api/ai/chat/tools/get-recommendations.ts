...
import type { AccountType, Holding, Profile, Stock } from "@prisma/client";
...
function filterStockData(
    response: ResolvedPromise<ReturnType<typeof fetchOptimalPortfolio>>,
    adjustedTransactions: FormattedTransaction[]
) {
    return response.stockData.map((stock) => {
        if (adjustedTransactions.find((transaction) => transaction.stockId === stock.id)) {
            const expectedReturn = stock.priceTarget && stock.previousClose? (stock.priceTarget / stock.previousClose) - 1: null;
            return {
                symbol: stock.symbol,
                name: stock.name,
                ...
                beta: stock.beta,
                stdDev: stock.stdDev,
                expectedReturn: expectedReturn? (100 * expectedReturn).toFixed(2): "N/A",
            }
        }
...
export async function getRecommendations(
    ...
    try {
        ...
        const stockData = filterStockData(response, adjustedTransactions);
...
export async function* handleRecommendations({
    ...
    const { textStream } = await streamText({
        model: openai('gpt-4o'),
        system: accountType === "STUDENT"? (
`You are assisting an investment-focused chatbot in providing a list of recommended transactions for the user. 
The user is a high-school student participating in the ASX Sharemarket Game. The Game runs for 3 months. 
Students receive 50,000 AUD of in-hame currency to buy and sell shares listed on the ASX with the goal of outperforming other students. 
You will receive a list of recommended transactions that have been generated by a portfolio optimisation algorithm. 
